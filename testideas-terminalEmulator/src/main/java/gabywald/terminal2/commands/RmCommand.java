...
public class RmCommand implements Command {
    ...
    public RmCommand(FileSystem fileSystem) { this.fileSystem = fileSystem; }

    @Override
    public String execute(String[] args, String stdin) {
        if (args.length == 0) { return "Usage: rm <filename|directory>"; }
        if (this.fileSystem.rm(args[0])) { return ""; }
