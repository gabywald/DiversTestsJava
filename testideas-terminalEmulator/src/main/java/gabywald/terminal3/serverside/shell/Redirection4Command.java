 ...
 */
public class Redirection4Command {
    /**
     * Type de redirection.
     */
    public enum Type {
     ...
     */
    public Redirection4Command(Type type, String fileName) {
        this.type = type;
     ...
     */
    public static List<Redirection4Command> parseRedirections(String[] args) {
        List<Redirection4Command> redirections = new ArrayList<>();
        List<String> cleanedArgs = new ArrayList<>();

        for (int i = 0; i < args.length; i++) {
            String arg = args[i];
            if (arg.equals(">") && i + 1 < args.length) {
                redirections.add(new Redirection4Command(Type.OUTPUT, args[i + 1]));
                i++;
            } else if (arg.equals(">>") && i + 1 < args.length) {
                redirections.add(new Redirection4Command(Type.APPEND, args[i + 1]));
                i++;
            } else if (arg.equals("<") && i + 1 < args.length) {
                redirections.add(new Redirection4Command(Type.INPUT, args[i + 1]));
                i++;
