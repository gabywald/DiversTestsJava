...
import gabywald.tests.http.BetterLogger.LOG_LEVEL;
...
public class HttpConnection {
	...
	private static final BetterLogger blogger = BetterLogger.getLogger(LOG_LEVEL.INFO);
...
//				wr.close();
			...
			HttpConnection.blogger.error("MalformedURLException: {" + e.getMessage() + "}");
			e.printStackTrace();
		} catch (UnsupportedEncodingException e) {
			HttpConnection.blogger.error("UnsupportedEncodingException: {" + e.getMessage() + "}");
			e.printStackTrace();
		} catch (IOException e) {
			HttpConnection.blogger.error("IOException: {" + e.getMessage() + "}");
			e.printStackTrace();
		}
	...
	private int showOutputFrom(HttpURLConnection connection) throws IOException {
		HttpConnection.blogger.debug("getContentLength: [" + connection.getContentLength() + "]" );
		
		int responseCode	= connection.getResponseCode();
		String outMessage	= connection.getResponseMessage();
		
		HttpConnection.blogger.debug("responseCode: [" + responseCode + "]" );
		HttpConnection.blogger.debug("outMessage--: {" + outMessage + "}" );
		
		// TODO make better HTTP codes recognition !
		if (responseCode != HttpStatus.SC_OK) {
			HttpConnection.blogger.warn("responseCode: [" + responseCode + "]" );
			HttpConnection.blogger.warn("outMessage--: {" + outMessage + "}" );
			...
			while ((inputLine = in.readLine()) != null) {
				HttpConnection.blogger.warn(inputLine );
		...
		while (iteOnMapKeys.hasNext()) {
			...
			HttpConnection.blogger.debug("\t KEY: {" + currentKey + "}" );
			List<String> values = responseHeaders.get( currentKey );
			for (int i = 0 ; i < values.size() ; i++) {
				HttpConnection.blogger.debug("\t\t (" + i + "): {" + values.get( i ) + "}" );
			}
		...
		// StringBuffer response = new StringBuffer();
		while ((inputLine = in.readLine()) != null) {
			HttpConnection.blogger.debug(inputLine );
